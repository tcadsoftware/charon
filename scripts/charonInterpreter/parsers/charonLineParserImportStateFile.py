...
class charonLineParserImportStateFile:
    "ImportStateFile parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "ImportStateFile"
        self.parsingKey = "import state file"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("at index")
        self.interpreterHelpLine = "Import State File {filename} [at Index {index}] "
        self.interpreterQuickHelp = "Specify state file name and state index"
        ...
        self.xmlRequiredLines.append("Charon->Mesh->Exodus File,File Name,string,{filename}")
        ...
        self.xmlOptionalLines[0].append(" Charon->Mesh->Exodus File,Restart Index,int,{index}")
        self.xmlOptionalLinePriority[0].append(2)

        # Register the xml optional arguments and their indexes
        self.xmlOptionalArgument = [['{index}']]
        self.xmlOptionalArgumentIndexes = [[2]]

        # Register the xml default lines
        self.xmlDefaultLines = []
        self.xmlDefaultLinePriority = []
        self.xmlDefaultLines.append(" Charon->Mesh,Source,string,Exodus File")
        self.xmlDefaultLines.append(" Charon->Mesh->Exodus File,Restart Index,int,-1")
