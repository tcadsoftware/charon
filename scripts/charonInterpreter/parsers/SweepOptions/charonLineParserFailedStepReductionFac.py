...
class charonLineParserFailedStepReductionFac:
    "FailedStepReductionFac parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "FailedStepReductionFac"
        self.parsingKey = "failed step reduction factor"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "failed step reduction factor = {factor} "
        self.interpreterQuickHelp = "Specify failed step reduction factor in modifying step size during parameter sweep"
        self.interpreterLongHelp = "Specify failed step reduction factor in modifying step size during parameter sweep"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Solution Control->LOCA->Step Size,Failed Step Reduction Factor,double,{factor}")
