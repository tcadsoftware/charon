...
class charonLineParserMaximumNumberSteps:
    "MaximumNumberSteps parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "MaximumNumberSteps"
        self.parsingKey = "maximum number of steps"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "Maximum number of steps = {numSteps} "
        self.interpreterQuickHelp = "Specify maximum number of steps allowed in the sweep"
        self.interpreterLongHelp = "Specify maximum number of steps allowed in the sweep <> {numSteps} = the number of steps"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Solution Control->LOCA->Stepper,Max Steps,int,{numSteps}")
        self.xmlRequiredLinePriority.append(5)
        self.xmlNewRequiredLines = []

        # Register the xml required arguments and their indexes
        self.xmlRequiredArgument = []
        self.xmlRequiredArgument.append("{numSteps}")
