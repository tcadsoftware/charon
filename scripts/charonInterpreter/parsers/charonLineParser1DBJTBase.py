...
class charonLineParser1DBJTBase:
    "1DBJTBase parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "1DBJTBase"
        self.parsingKey = "bc is 1dbjt for"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("fixed at")
        self.parsingKeyOptional.append("swept from")
        self.interpreterHelpLine = "BC is 1dbjt for {baseType} {sidesetID} on {geometryBlock} [fixed at {potential}[ swept from {potential1} to {potential2}]] "
        self.interpreterQuickHelp = "Specify the potential on a 1D BJT base contact"
        self.interpreterLongHelp = "Specify the potential on a 1D BJT base contact. <> baseType is the doping type in the base (Acceptor/Donor) <> sidesetID is the contact name/type <> geometryBlock is the geometry name the contact is attached to <> potential is the value in volts"
        ...
        self.xmlRequiredLines.append("Charon->Boundary Conditions->{sidesetID}ANONYMOUS,Strategy,string,BJT1D Base Contact")
        self.xmlRequiredLines.append("Charon->Boundary Conditions->{sidesetID}ANONYMOUS->Data,Base Doping Type,string,{baseType}")
        ...
        self.xmlRequiredArgument.append("{baseType}")
