...
class charonLineParserInitialConditions:
    "InitialConditions parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "InitialConditions"
        self.parsingKey = "initial conditions for"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("exodus file")
        self.parsingKeyOptional.append("equilibrium density")
        self.parsingKeyOptional.append("equilibrium potential")
        self.parsingKeyOptional.append("constant")
        self.parsingKeyOptional.append("uninitialized")
        self.interpreterHelpLine = "Initial Conditions for {DOF} in {geometryBlock} is [ Exodus File[ equilibrium density [ equilibrium potential [constant = {constValue} [uninitialized ]]]]] "
        self.interpreterQuickHelp = "Specify the source of the initial condition for the degrees of freedom"
        self.interpreterLongHelp = "Specify the source of the initial condition for the degrees of freedom <> Degrees of freedoms are <> ELECTRIC_POTENTIAL <> ELECTRON_DENSITY <> HOLE_DENSITY <> initial conditions are: <> Exodus File <> Equilibrium Density"
        ...
        self.xmlRequiredArgument.append("{DOF}")
        ...
        self.xmlOptionalLines[0].append(" Charon->Initial Conditions->{geometryBlock}->{DOF},Value,string,Exodus File")
        self.xmlOptionalLines[0].append(" use Modifier 1")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[1].append(" Charon->Initial Conditions->{geometryBlock}->{DOF},Value,string,Equilibrium Density")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[2].append(" Charon->Initial Conditions->{geometryBlock}->{DOF},Value,string,Equilibrium Potential")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[3].append(" Charon->Initial Conditions->{geometryBlock}->{DOF},Value,double,{constValue}")
        self.xmlOptionalLines[3].append(" use Modifier 0")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[4].append(" Charon->Initial Conditions->{geometryBlock}->{DOF},Value,double,0.0")
        ...
        self.xmlOptionalArgument = [[], [], [], ['{constValue}'], []]
