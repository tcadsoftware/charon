...
class charonLineParserContactOnInsulator:
    "ContactOnInsulator parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "ContactOnInsulator"
        self.parsingKey = "bc is contact on insulator for"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("fixed at")
        self.parsingKeyOptional.append("swept from")
        self.interpreterHelpLine = "BC is contact on insulator for {sidesetID} on {geometryBlock} with work function {workFunction} [fixed at {potential}[ swept from {potential1} to {potential2}]] "
        ...
        self.xmlRequiredLines.append("Charon->Boundary Conditions->{sidesetID}ANONYMOUS,Equation Set Name,string,ELECTRIC_POTENTIAL")
        self.xmlRequiredLines.append("Charon->Boundary Conditions->{sidesetID}ANONYMOUS,Strategy,string,Contact On Insulator")
        self.xmlRequiredLines.append("Charon->Boundary Conditions->{sidesetID}ANONYMOUS->Data,Work Function,double,{workFunction}")
        ...
        self.xmlRequiredArgument.append("{workFunction}")
