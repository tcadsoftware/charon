...
class charonLineParserOutputStateFile:
    "OutputStateFile parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "OutputStateFile"
        self.parsingKey = "output state file"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "Output State File {filename} "
        self.interpreterQuickHelp = "Specify state file name for output"
        self.interpreterLongHelp = "Specify the name of the file to which the states are written <> filename is the name of the file.  It is case sensitive."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Output,File Name,string,{filename}")
