...
class charonLineParserTabulatedTransientCurrents:
    "TabulatedTransientCurrents parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "TabulatedTransientCurrents"
        self.parsingKey = "output tabulated transient"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "Output tabulated transient currents to {filename} "
        self.interpreterQuickHelp = "Output tabulated transient currents to the file {filename}"
        self.interpreterLongHelp = "Output tabulated transient currents to the file {filename} <> filename is the name of the file.  It is case sensitive."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Solver Factories->Rythmos Observers,Output Responses,string,ON")
        self.xmlRequiredLines.append("Charon->Solver Factories->Rythmos Observers,Write Response File,string,{filename}")
