...
class charonLineParserTabulatedParameterCurrents:
    "TabulatedParameterCurrents parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "TabulatedParameterCurrents"
        self.parsingKey = "output tabulated parameter"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "Output tabulated parameter currents to {filename} "
        self.interpreterQuickHelp = "Output tabulated parameter currents to the file {filename}"
        self.interpreterLongHelp = "Output tabulated parameter currents to the file {filename} <> filename is the name of the file.  It is case sensitive."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Solver Factories->NOX Observers,Output Responses,string,ON")
        self.xmlRequiredLines.append("Charon->Solver Factories->NOX Observers,Output Responses File,string,{filename}")
