...
class charonLineParserFixedCharge:
    "FixedCharge parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "FixedCharge"
        self.parsingKey = "fixed charge"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("=")
        self.parsingKeyOptional.append("is swept from")
        self.interpreterHelpLine = "fixed charge [= {fixedCharge} [ is swept from {charge1} to {charge2}]] "
        self.interpreterQuickHelp = "Specify the fixed charge at an interface in unit of cm^-2"
        self.interpreterLongHelp = "Specify the fixed charge at an interface in unit of cm^-2 <> Charge can be set as a single value <> or it can be swept from one value to another"
        ...
        self.xmlOptionalLines[0].append(" Charon->Boundary Conditions->{sidesetID}ANONYMOUS->Data,Fixed Charge,double,{fixedCharge}")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[1].append(" Charon->Boundary Conditions->{sidesetID}ANONYMOUS->Data,Varying Charge,string,Parameter")
        ...
        self.xmlOptionalLines[1].append(" Charon->Solution Control->LOCA->Stepper,Initial Value,double,{charge1}")
        self.xmlOptionalLines[1].append(" Charon->Solution Control->LOCA->Stepper,Continuation Parameter,string,Varying Charge")
        self.xmlOptionalLines[1].append(" Charon->Solution Control->LOCA->Stepper,Max Steps,int,1000")
        self.xmlOptionalLines[1].append(" Charon->Solution Control->LOCA->Stepper,Max Value,double,{charge2}")
        self.xmlOptionalLines[1].append(" Charon->Solution Control->LOCA->Stepper,Min Value,double,{charge1}")
        ...
        self.xmlOptionalLines[1].append(" Charon->Active Parameters->Parameter Vector 0,Parameter 0,string,Varying Charge")
        self.xmlOptionalLines[1].append(" Charon->Active Parameters->Parameter Vector 0,Initial Value 0,double,{charge1}")
        ...
        self.xmlOptionalArgument = [['{fixedCharge}'], ['{charge1}', '{charge2}']]
