...
class charonLineParserTrapEnergy:
    "TrapEnergy parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "TrapEnergy"
        self.parsingKey = "energy ="
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "energy = {trapEnergy} "
        self.interpreterQuickHelp = "Specify trap energy in eV with respect to the intrinsic Fermi level Ei"
        self.interpreterLongHelp = "Specify trap energy in eV with respect to the intrinsic Fermi level Ei. The trap energy is positive above Ei  and negative below Ei."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Boundary Conditions->{sidesetID}ANONYMOUS->Data->Surface Trap->Trap {trapID},Trap Energy,double,{trapEnergy}")
        ...
        self.xmlRequiredArgument.append("{trapEnergy}")
