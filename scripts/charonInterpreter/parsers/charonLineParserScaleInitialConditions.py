...
class charonLineParserScaleInitialConditions:
    "ScaleInitialConditions parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "ScaleInitialConditions"
        self.parsingKey = "scale initial conditions"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("is on")
        self.parsingKeyOptional.append("is off")
        self.interpreterHelpLine = "scale initial conditions [ is on [ is off ]] "
        self.interpreterQuickHelp = "Specify whether the initial conditions  as read from an exodus file  should be scaled"
        self.interpreterLongHelp = "Specify whether the initital conditions  as read from an exodus file  should be scaled <> By default all charon output is scaled but that can be turned off. In order to use such <> a file for initial conditions the DOFs must be scaled"
        ...
        self.xmlOptionalLines[0].append(" Charon->Initial Conditions,Scale Input Exodus Variables,bool,true")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[1].append(" Charon->Initial Conditions,Scale Input Exodus Variables,bool,false")
