...
class charonLineParserDirectXMLInput:
    "DirectXMLInput parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "DirectXMLInput"
        self.parsingKey = "direct input"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "Direct input {inputFodder} "
        self.interpreterQuickHelp = "Input directly a quoted  LCM formatted parameter\" into the xml parameter list."
        self.interpreterLongHelp = "Input directly a quoted \"LCM formatted parameter\" into the xml parameter list. <> The LCM format is:  Nest0->Nest1->Nest2...Nestn parameterName parameterDataType parameterValue"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("{inputFodder}")
        self.xmlRequiredLinePriority.append(100)
        self.xmlNewRequiredLines = []

        # Register the xml required arguments and their indexes
        self.xmlRequiredArgument = []
        self.xmlRequiredArgument.append("{inputFodder}")
