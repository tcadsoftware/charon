...
class charonLineParserMuDataFile:
    "MuDataFile parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "MuDataFile"
        self.parsingKey = "mu table data file is"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "mu table data file is {filename} "
        self.interpreterQuickHelp = "set the name of the mu data file."
        self.interpreterLongHelp = "set the name of the mu data file. <> The empirical model relies on mu values froma tabulated file.  et the file name here."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->Empirical Defect Recombination,mu data file,string,{filename}")
