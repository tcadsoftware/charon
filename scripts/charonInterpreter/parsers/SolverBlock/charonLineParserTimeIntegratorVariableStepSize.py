...
class charonLineParserTimeIntegratorVariableStepSize:
    "TimeIntegratorVariableStepSize parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "TimeIntegratorVariableStepSize"
        self.parsingKey = "time integrator variable step size"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("with max step")
        self.interpreterHelpLine = "time integrator variable step size [with max step = {dtMax}] "
        self.interpreterQuickHelp = "Allow variable time stepping for the time intergrator."
        self.interpreterLongHelp = "Allow variable time stepping for the time integrator. <> dtMax is optionally provided as the maximum allowed time step size in seconds."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Solution Control->Rythmos->Rythmos Integration Control,Take Variable Steps,bool,True")
        ...
        self.xmlOptionalLines[0].append(" Charon->Solution Control->Rythmos->Rythmos Integration Control,Max dt,double,{dtMax}")
        self.xmlOptionalLinePriority[0].append(2)

        # Register the xml optional arguments and their indexes
        self.xmlOptionalArgument = [['{dtMax}']]
        self.xmlOptionalArgumentIndexes = [[4]]
