...
class charonLineParserTimeIntegratorMinTimeStepSize:
    "TimeIntegratorMinTimeStepSize parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "TimeIntegratorMinTimeStepSize"
        self.parsingKey = "time integrator minimum step size"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "time integrator minimum step size = {dtime} "
        self.interpreterQuickHelp = "Specify the minimum time step size the intergrator is allowed to take."
        self.interpreterLongHelp = "Specify the minimum time step size the intergrator is allowed to take. <> dtime is the time step in seconds."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Solution Control->Rythmos->Rythmos Step Control Strategy,Min Step Size,double,{dtime}")
