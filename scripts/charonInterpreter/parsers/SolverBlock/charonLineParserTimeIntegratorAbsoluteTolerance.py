...
class charonLineParserTimeIntegratorAbsoluteTolerance:
    "TimeIntegratorAbsoluteTolerance parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "TimeIntegratorAbsoluteTolerance"
        self.parsingKey = "time integrator absolute error"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "time integrator absolute error = {error} "
        self.interpreterQuickHelp = "Specify the time integrator absolute error tolerance"
        self.interpreterLongHelp = "Specify time integrator absolute error tolerance <> {error = target value for the absolute error requirement"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Solution Control->Rythmos->Rythmos Step Control Strategy,Absolute Error Tolerance,double,{error}")
