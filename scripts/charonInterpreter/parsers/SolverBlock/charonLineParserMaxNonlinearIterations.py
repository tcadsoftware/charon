...
class charonLineParserMaxNonlinearIterations:
    "MaxNonlinearIterations parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "MaxNonlinearIterations"
        self.parsingKey = "max nonlinear iterations"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "max nonlinear iterations = {maxIterations} "
        self.interpreterQuickHelp = "Specify the maximum number of non linear iterations before failure is declared"
        self.interpreterLongHelp = "Specify the maximum number of non linear iterations before failure is declared <> maxIterations = maximum number of iterations"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Solution Control->NOX->Status Tests->Test 1,Maximum Iterations,int,{maxIterations}")
