...
class charonLineParserTimeIntegratorInitialStepSize:
    "TimeIntegratorInitialStepSize parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "TimeIntegratorInitialStepSize"
        self.parsingKey = "time integrator initial step size"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "time integrator initial step size = {dtime} "
        self.interpreterQuickHelp = "Specify the initial time step size the intergrator is allowed to take."
        self.interpreterLongHelp = "Specify the initial time step size the intergrator is allowed to take. <> dtime is the time step in seconds."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Solution Control->Rythmos->Rythmos Step Control Strategy,Initial Step Size,double,{dtime}")
