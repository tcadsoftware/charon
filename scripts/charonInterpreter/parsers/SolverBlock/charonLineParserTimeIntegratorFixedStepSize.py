...
class charonLineParserTimeIntegratorFixedStepSize:
    "TimeIntegratorFixedStepSize parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "TimeIntegratorFixedStepSize"
        self.parsingKey = "time integrator fixed step size"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "time integrator fixed step size = {dtime} "
        self.interpreterQuickHelp = "Specify the fixed time step size the intergrator will take."
        self.interpreterLongHelp = "Specify the fixed time step size the intergrator will take. <> dtime is the time step in seconds."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Solution Control->Rythmos->Rythmos Integration Control,Fixed dt,double,{dtime}")
