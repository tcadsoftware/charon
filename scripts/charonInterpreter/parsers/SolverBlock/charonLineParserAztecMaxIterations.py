...
class charonLineParserAztecMaxIterations:
    "AztecMaxIterations parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "AztecMaxIterations"
        self.parsingKey = "aztec max iterations"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "aztec max iterations = {maxIterations} "
        self.interpreterQuickHelp = "Specify Aztec maximum number of linear iterations before failure is declared"
        self.interpreterLongHelp = "Specify Aztec maximum number of linear iterations before failure is declared <> maxIterations = maximum number of iterations"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Solution Control->NOX->Direction->Newton->Stratimikos Linear Solver->Stratimikos->Linear Solver Types->AztecOO->Forward Solve,Max Iterations,int,{maxIterations}")
        ...
        self.xmlRequiredArgument.append("{maxIterations}")
