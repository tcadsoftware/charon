...
class charonLineParserInputMeshScaleFactor:
    "InputMeshScaleFactor parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "InputMeshScaleFactor"
        self.parsingKey = "input mesh scale factor"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "input mesh scale factor = {sfval} "
        self.interpreterQuickHelp = "Apply a scale factor to all coordinate values read from the input file"
        self.interpreterLongHelp = "Apply a scale factor to all coordinate values read from the input file"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Mesh->Exodus File,Scale Factor,double,{sfval}")
        ...
        self.xmlRequiredArgument.append("{sfval}")
