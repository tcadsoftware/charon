...
class charonBlockParserEffectiveDOS:
    "EffectiveDOS parser"

    def __init__(self):
        # Register the parsing keys
        self.parsingBlockKey = "start effective dos"
        self.parserBlockName = "EffectiveDOS"

        # Register the block arguments
        self.blockArgument = []
        self.blockArgumentIndexes = []
        self.interpreterBlockHelpLine = "start Effective DOS "

        # Register the xml default lines
        self.xmlDefaultLines = []
        self.xmlDefaultLines.append("Charon->Closure Models->{MaterialBlockName}->Effective DOS,Value,string,Simple")
