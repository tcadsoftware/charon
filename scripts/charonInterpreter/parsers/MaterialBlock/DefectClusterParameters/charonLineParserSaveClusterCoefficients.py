...
class charonLineParserSaveClusterCoefficients:
    "SaveClusterCoefficients parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "SaveClusterCoefficients"
        self.parsingKey = "save cluster coefficients is"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("on")
        self.parsingKeyOptional.append("off")
        self.interpreterHelpLine = "Save cluster coefficients is [On [ Off ]] "
        self.interpreterQuickHelp = "Save the cluster coefficients in a file"
        self.interpreterLongHelp = "Save the cluster coefficients in a file <> This is useful if the analyst wishes to see local effects of the radiation damage."
        ...
        self.xmlOptionalLines[0].append(" Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Save Cluster Coefficients,bool,True")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[1].append(" Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Save Cluster Coefficients,bool,False")
