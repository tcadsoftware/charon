...
class charonLineParserInterpolantMethod:
    "InterpolantMethod parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "InterpolantMethod"
        self.parsingKey = "interpolant method is"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("1d linear")
        self.parsingKeyOptional.append("shepard 1d")
        self.parsingKeyOptional.append("shepard 2d")
        self.parsingKeyOptional.append("shepard 3d")
        self.interpreterHelpLine = "Interpolant method is [ 1d linear [shepard 1d [shepard 2d [ shepard 3d ]]]] "
        self.interpreterQuickHelp = "set the interpolate scheme for the recombination factor due to defects"
        self.interpreterLongHelp = "set the interpolate scheme for the recombination factor due to defects <> Options are 1d linear  shepard 1d  shepard 2d  Shepard 3d"
        ...
        self.xmlOptionalLines[0].append(" Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Interpolant Method,string,ONED_LINEAR_INTERPOLATION")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[1].append(" Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Interpolant Method,string,SHEPARDS_1D")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[2].append(" Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Interpolant Method,string,SHEPARDS_2D")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[3].append(" Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Interpolant Method,string,SHEPARDS_3D")
