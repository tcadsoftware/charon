...
class charonLineParserPulseFile:
    "PulseFile parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "PulseFile"
        self.parsingKey = "pulse file is"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "Pulse file is {pulseFile} "
        self.interpreterQuickHelp = "Set the pulse details file name for in situ clustering models"
        self.interpreterLongHelp = "Set the pulse details file name for in situ clustering models. <> This file contains cluster densities vs time over a radiation pulse waveform."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Finite Pulse,string,{pulseFile}")
        ...
        self.xmlRequiredArgument.append("{pulseFile}")
