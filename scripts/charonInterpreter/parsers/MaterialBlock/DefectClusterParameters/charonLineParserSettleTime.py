...
class charonLineParserSettleTime:
    "SettleTime parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "SettleTime"
        self.parsingKey = "settle time"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "Settle time = {settleTime} "
        self.interpreterQuickHelp = "Set the time over which the cluster settles out due to sharp impulse effects of damage."
        self.interpreterLongHelp = "Set the time over which the cluster settles out due to sharp impulse effects of damage.  <> Usually the cluster behavior is very erratic over the initial 100-1000 ps as defects ionize.  <> This can harm a simulation more than help.  This time setting ignores that initial damage onset."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Cluster Wonky Time,double,{settleTime}")
        ...
        self.xmlRequiredArgument.append("{settleTime}")
