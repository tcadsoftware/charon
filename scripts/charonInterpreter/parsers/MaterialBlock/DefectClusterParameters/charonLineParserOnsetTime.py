...
class charonLineParserOnsetTime:
    "OnsetTime parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "OnsetTime"
        self.parsingKey = "onset time"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "Onset time = {onsetTime} "
        self.interpreterQuickHelp = "Set the time relative to time zero that the radiation pulse starts"
        self.interpreterLongHelp = "Set the time that the radiation pulse starts <> This is relative to the start of the simulation--most often relative to zero."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Cluster Onset Time,double,{onsetTime}")
        ...
        self.xmlRequiredArgument.append("{onsetTime}")
