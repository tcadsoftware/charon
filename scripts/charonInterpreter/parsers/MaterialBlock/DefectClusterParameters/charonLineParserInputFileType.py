...
class charonLineParserInputFileType:
    "InputFileType parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "InputFileType"
        self.parsingKey = "cluster data file type"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("1d data")
        self.parsingKeyOptional.append("2d data")
        self.parsingKeyOptional.append("3d data")
        self.parsingKeyOptional.append("in situ")
        self.interpreterHelpLine = "cluster data file type is [ 1d data [ 2d data [ 3d data [ in situ ]]]] "
        self.interpreterQuickHelp = "set the input file type for tabulated cluster data"
        self.interpreterLongHelp = "set the input file type for tabulated cluster dataset the cascade density in 1/cm^3 <> options are 1d data  2d data  3d data"
        ...
        self.xmlOptionalLines[0].append(" Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Input File Type,string,CLUSTER1DX")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[1].append(" Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Input File Type,string,CLUSTER2D")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[2].append(" Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Input File Type,string,CLUSTER3D")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[3].append(" Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Input File Type,string,IN SITU")
        self.xmlOptionalLines[3].append(" Charon->Solver Factories->Rythmos Observers,Output Responses,string,ON")
        self.xmlOptionalLines[3].append(" Charon->Solver Factories->Rythmos Observers,Write Response File,string,current_time_dd_pulse.csv")
        self.xmlOptionalLines[3].append(" Charon->Solver Factories->Rythmos Observers,Cluster Observer,string,ON")
        self.xmlOptionalLines[3].append(" Charon->Solver Factories->Rythmos Observers,Cluster Observer,string,ON")
        self.xmlOptionalLines[3].append(" Charon->Closure Models->siliconParameter->Defect Cluster Recombination,Update Cluster BCs,string,never")
