...
class charonLineParserSingleFileClusters:
    "SingleFileClusters parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "SingleFileClusters"
        self.parsingKey = "single file clusters is"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("on")
        self.parsingKeyOptional.append("off")
        self.interpreterHelpLine = "single file clusters is [ On [ Off ]] "
        self.interpreterQuickHelp = "Toggle whether the clusters are lined up in a single file"
        self.interpreterLongHelp = "Toggle whether the clusters are lined up in a single file <> This will only be true in a quasi-one D simulation"
        ...
        self.xmlOptionalLines[0].append(" Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Single File Clusters,bool,True")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[1].append(" Charon->Closure Models->{MaterialBlockName}->Defect Cluster Recombination,Single File Clusters,bool,False")
