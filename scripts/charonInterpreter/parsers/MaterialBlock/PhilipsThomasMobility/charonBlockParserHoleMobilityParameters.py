...
class charonBlockParserHoleMobilityParameters:
    "HoleMobilityParameters parser"

    def __init__(self):
        # Register the parsing keys
        self.parsingBlockKey = "start hole parameters"
        self.parserBlockName = "HoleMobilityParameters"

        # Register the block arguments
        self.blockArgument = []
        self.blockArgumentIndexes = []
        self.interpreterBlockHelpLine = "start hole parameters "

        # Register the xml default lines
        self.xmlDefaultLines = []
        self.xmlDefaultLines.append("Charon->Closure Models->{MaterialBlockName}->Hole Mobility,Value,string,Philips-Thomas")
