...
class charonBlockParserElectronMobilityParameters:
    "ElectronMobilityParameters parser"

    def __init__(self):
        # Register the parsing keys
        self.parsingBlockKey = "start electron parameters"
        self.parserBlockName = "ElectronMobilityParameters"

        # Register the block arguments
        self.blockArgument = []
        self.blockArgumentIndexes = []
        self.interpreterBlockHelpLine = "start electron parameters "

        # Register the xml default lines
        self.xmlDefaultLines = []
        self.xmlDefaultLines.append("Charon->Closure Models->{MaterialBlockName}->Electron Mobility,Value,string,Philips-Thomas")
