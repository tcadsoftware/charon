...
class charonLineParserHoleLifetime:
    "HoleLifetime parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "HoleLifetime"
        self.parsingKey = "hole lifetime is constant"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "hole lifetime is constant = {lifetime} "
        self.interpreterQuickHelp = "Specify an explicit value for hole lifetime."
        self.interpreterLongHelp = "Specify an explicit value for hole lifetime. <> {lifetime} is the lifetime in seconds."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Hole Lifetime,Value,double,{lifetime}")
