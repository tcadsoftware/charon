...
class charonLineParserElectronConcentrationDependentFunction:
    "ElectronConcentrationDependentFunction parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "ElectronConcentrationDependentFunction"
        self.parsingKey = "electron lifetime is concentration"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("with nsrh")
        self.interpreterHelpLine = "electron lifetime is concentration dependent [with Nsrh = {Nsrh}] "
        self.interpreterQuickHelp = "Specify that the electron lifetime is concentration dependent."
        self.interpreterLongHelp = "Specify that the electron lifetime is concentration dependent. <> {Nsrh} is the parameter for the function."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Electron Lifetime,Value,string,Function")
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Electron Lifetime->Function 1,Function Type,string,ConcDep")
        ...
        self.xmlOptionalLines[0].append(" Charon->Closure Models->{MaterialBlockName}->Electron Lifetime->Function 1,Nsrh,double,{Nsrh}")
        self.xmlOptionalLinePriority[0].append(2)

        # Register the xml optional arguments and their indexes
        self.xmlOptionalArgument = [['{Nsrh}']]
        self.xmlOptionalArgumentIndexes = [[3]]
