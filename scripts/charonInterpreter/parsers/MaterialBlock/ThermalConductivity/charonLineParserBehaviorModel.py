...
class charonLineParserBehaviorModel:
    "BehaviorModel parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "BehaviorModel"
        self.parsingKey = "behavior model"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("tempdep")
        self.parsingKeyOptional.append("powerlawtempdep")
        self.parsingKeyOptional.append("lineartempdep")
        self.parsingKeyOptional.append("lineariondep")
        self.interpreterHelpLine = "behavior model is [ tempdep [ powerlawtempdep [ lineartempdep [ lineariondep ]]]] "
        self.interpreterQuickHelp = "Set behavior model for thermal conductivity"
        self.interpreterLongHelp = "Set behavior model for thermal conductivity"
        ...
        self.xmlOptionalLines[0].append(" Charon->Closure Models->{MaterialBlockName}->Thermal Conductivity,Value,string,TempDep")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[1].append(" Charon->Closure Models->{MaterialBlockName}->Thermal Conductivity,Value,string,PowerLawTempDep")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[2].append(" Charon->Closure Models->{MaterialBlockName}->Thermal Conductivity,Value,string,LinearTempDep")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[3].append(" Charon->Closure Models->{MaterialBlockName}->Thermal Conductivity,Value,string,LinearIonDep")
