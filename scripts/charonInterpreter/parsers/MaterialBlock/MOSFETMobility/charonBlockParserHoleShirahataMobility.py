...
class charonBlockParserHoleShirahataMobility:
    "HoleShirahataMobility parser"

    def __init__(self):
        # Register the parsing keys
        self.parsingBlockKey = "start hole channel mobility shirahata"
        self.parserBlockName = "HoleShirahataMobility"

        # Register the block arguments
        self.blockArgument = []
        self.blockArgumentIndexes = []
        self.interpreterBlockHelpLine = "start hole channel mobility shirahata "

        # Register the xml default lines
        self.xmlDefaultLines = []
        self.xmlDefaultLines.append("Charon->Closure Models->siliconParameter->Hole Mobility->Perpendicular Field Model,Value,string,Shirahata")
