...
class charonBlockParserHoleBulkPhilipsThomasMobility:
    "HoleBulkPhilipsThomasMobility parser"

    def __init__(self):
        # Register the parsing keys
        self.parsingBlockKey = "start hole bulk mobility klaassen"
        self.parserBlockName = "HoleBulkPhilipsThomasMobility"

        # Register the block arguments
        self.blockArgument = []
        self.blockArgumentIndexes = []
        self.interpreterBlockHelpLine = "start hole bulk Mobility Klaassen "

        # Register the xml default lines
        self.xmlDefaultLines = []
        self.xmlDefaultLines.append("Charon->Closure Models->siliconParameter->Hole Mobility,Value,string,MOSFET")
        self.xmlDefaultLines.append("Charon->Closure Models->siliconParameter->Hole Mobility->Bulk Mobility,Value,string,Klaassen")
        self.xmlDefaultLines.append("Charon->Closure Models->{MaterialBlockName}->Hole Mobility->Bulk Mobility,G Parameter Set,string,Klaassen")
