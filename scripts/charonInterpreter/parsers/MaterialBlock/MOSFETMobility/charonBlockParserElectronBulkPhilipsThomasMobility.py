...
class charonBlockParserElectronBulkPhilipsThomasMobility:
    "ElectronBulkPhilipsThomasMobility parser"

    def __init__(self):
        # Register the parsing keys
        self.parsingBlockKey = "start electron bulk mobility klaassen"
        self.parserBlockName = "ElectronBulkPhilipsThomasMobility"

        # Register the block arguments
        self.blockArgument = []
        self.blockArgumentIndexes = []
        self.interpreterBlockHelpLine = "start electron bulk Mobility Klaassen "

        # Register the xml default lines
        self.xmlDefaultLines = []
        self.xmlDefaultLines.append("Charon->Closure Models->siliconParameter->Electron Mobility,Value,string,MOSFET")
        self.xmlDefaultLines.append("Charon->Closure Models->siliconParameter->Electron Mobility->Bulk Mobility,Value,string,Klaassen")
        self.xmlDefaultLines.append("Charon->Closure Models->{MaterialBlockName}->Electron Mobility->Bulk Mobility,G Parameter Set,string,Klaassen")
