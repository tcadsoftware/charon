...
class charonLineParserDopingRamp:
    "DopingRamp parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "DopingRamp"
        self.parsingKey = "doping ramps from"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "Doping ramps from {startConcentration} to {stopConcentration} "
        self.interpreterQuickHelp = "set the ramp of the doping over the linear range specified."
        self.interpreterLongHelp = "set the ramp of the doping over the linear range specified. <> startConcentration is the concentraiton at the start range specified in 1/cm^3 <> stopConcentration is the concentration at the end of the ramp in 1/cm^3"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Doping->{functionName},Doping Start Value,double,{startConcentration}")
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Doping->{functionName},Doping End Value,double,{stopConcentration}")
        ...
        self.xmlRequiredArgument.append("{startConcentration}")
        self.xmlRequiredArgument.append("{stopConcentration}")
