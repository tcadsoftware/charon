...
class charonLineParserPeakTime:
    "PeakTime parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "PeakTime"
        self.parsingKey = "pulse peak time ="
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "Pulse peak time = {peakTime} "
        self.interpreterQuickHelp = "set the time when the pulse peaks"
        self.interpreterLongHelp = "set the time when the pulse peaks"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Optical Generation->{functionName},Time Peak Location,double,{peakTime}")
        ...
        self.xmlRequiredArgument.append("{peakTime}")
