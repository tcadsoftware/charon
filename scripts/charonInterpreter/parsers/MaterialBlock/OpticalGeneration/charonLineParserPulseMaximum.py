...
class charonLineParserPulseMaximum:
    "PulseMaximum parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "PulseMaximum"
        self.parsingKey = "maximum pulse rate ="
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "Maximum pulse rate = {pulseRate} "
        self.interpreterQuickHelp = "set the maximum hole/electron pair production rate of the pulse"
        self.interpreterLongHelp = "set the maximum hole/electron pair production rate of the pulse"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Optical Generation->{functionName},Max Value,double,{pulseRate}")
