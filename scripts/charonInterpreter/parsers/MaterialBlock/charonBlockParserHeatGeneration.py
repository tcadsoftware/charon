...
class charonBlockParserHeatGeneration:
    "HeatGeneration parser"

    def __init__(self):
        # Register the parsing keys
        self.parsingBlockKey = "start heat generation"
        self.parserBlockName = "HeatGeneration"

        # Register the block arguments
        self.blockArgument = []
        self.blockArgumentIndexes = []
        self.interpreterBlockHelpLine = "start heat generation "

        # Register the xml default lines
        self.xmlDefaultLines = []
        self.xmlDefaultLines.append("Charon->Closure Models->{MaterialBlockName}->Heat Generation,Value,string,Analytic")
