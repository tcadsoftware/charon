...
class charonLineParserStrikeStartTime:
    "StrikeStartTime parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "StrikeStartTime"
        self.parsingKey = "strike start time"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "strike start time = {time} "
        self.interpreterQuickHelp = "set the start time of the strike"
        self.interpreterLongHelp = "set the start time of the strike"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Particle Strike,Start Time,double,{time}")
