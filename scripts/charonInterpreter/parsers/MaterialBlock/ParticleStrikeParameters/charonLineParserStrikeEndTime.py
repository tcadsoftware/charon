...
class charonLineParserStrikeEndTime:
    "StrikeEndTime parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "StrikeEndTime"
        self.parsingKey = "strike end time"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "strike end time = {time} "
        self.interpreterQuickHelp = "set the end time of the strike"
        self.interpreterLongHelp = "set the end time of the strike"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Particle Strike,End Time,double,{time}")
