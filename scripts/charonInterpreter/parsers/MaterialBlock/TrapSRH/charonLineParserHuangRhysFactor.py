...
class charonLineParserHuangRhysFactor:
    "HuangRhysFactor parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "HuangRhysFactor"
        self.parsingKey = "huang-rhys factor"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "huang-rhys factor = {value} "
        self.interpreterQuickHelp = "Huang-Rhys factor"
        self.interpreterLongHelp = "Huang-Rhys factor"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Trap SRH Recombination->{trapName},Huang-Rhys Factor,double,{value}")
