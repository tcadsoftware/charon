...
class charonLineParserBandOffset:
    "BandOffset parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "BandOffset"
        self.parsingKey = "heterojunction band offset"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "heterojunction band offset = {value} "
        self.interpreterQuickHelp = "Bandgap offset"
        self.interpreterLongHelp = "Bandgap offset"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Trap SRH Recombination->{trapName},Electron HJ Band Offset,double,{value}")
