...
class charonLineParserLifetime:
    "Lifetime parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "Lifetime"
        self.parsingKey = "lifetime"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "lifetime = {value} "
        self.interpreterQuickHelp = "Carrier lifetime"
        self.interpreterLongHelp = "Carrier lifetime"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Trap SRH Recombination->{trapName},Electron Lifetime,double,{value}")
