...
class charonLineParserBGNFile:
    "BGNFile parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "BGNFile"
        self.parsingKey = "bgn file"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "bgn file = {fileName} "
        self.interpreterQuickHelp = "Specify a file that contains bgn values"
        self.interpreterLongHelp = "Specify a file that contains bgn values"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Intrinsic Concentration,BGN File,string,{fileName}")
