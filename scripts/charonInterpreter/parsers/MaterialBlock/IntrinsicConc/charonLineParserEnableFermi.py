...
class charonLineParserEnableFermi:
    "EnableFermi parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "EnableFermi"
        self.parsingKey = "enable fermi"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "enable fermi = {boolFlag}] "
        self.interpreterQuickHelp = "Specify the flag for enable fermi  either true or false"
        self.interpreterLongHelp = "Specify the flag for enable fermi  either true or false  which adds a Fermi-Dirac related correction to band gap narrowing value"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Intrinsic Concentration,Enable Fermi Correction,bool,{boolFlag}")
        ...
        self.xmlRequiredArgument.append("{boolFlag}]")
