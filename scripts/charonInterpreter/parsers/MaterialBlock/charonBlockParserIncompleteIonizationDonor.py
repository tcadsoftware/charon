...
class charonBlockParserIncompleteIonizationDonor:
    "IncompleteIonizationDonor parser"

    def __init__(self):
        # Register the parsing keys
        self.parsingBlockKey = "start incomplete ionization donor"
        self.parserBlockName = "IncompleteIonizationDonor"

        # Register the block arguments
        self.blockArgument = []
        self.blockArgumentIndexes = []
        self.interpreterBlockHelpLine = "start Incomplete Ionization Donor "

        # Register the xml default lines
        self.xmlDefaultLines = []
        self.xmlDefaultLines.append("Charon->Closure Models->{MaterialBlockName}->Incomplete Ionized Donor,Value,string,Model")
