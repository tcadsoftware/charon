...
class charonLineParserIntrinsicConcentrationModel:
    "IntrinsicConcentrationModel parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "IntrinsicConcentrationModel"
        self.parsingKey = "model for intrinsic concentration"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("medici")
        self.parsingKeyOptional.append("harmon")
        self.parsingKeyOptional.append("fermi corrected harmon")
        self.interpreterHelpLine = "model for intrinsic concentration is [medici [ harmon [ fermi corrected harmon ]]] "
        self.interpreterQuickHelp = "Set the intrinsic concentration model"
        ...
        self.xmlOptionalLines[0].append(" Charon->Closure Models->{MaterialBlockName}->Intrinsic Concentration,Value,string,Medici")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[1].append(" Charon->Closure Models->{MaterialBlockName}->Intrinsic Concentration,Value,string,Harmon")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[2].append(" Charon->Closure Models->{MaterialBlockName}->Intrinsic Concentration,Value,string,Harmon")
        self.xmlOptionalLines[2].append(" Charon->Closure Models->{MaterialBlockName}->Intrinsic Concentration,Enable Fermi Correction,bool,true")
