...
class charonLineParsermumin:
    "mumin parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "mumin"
        self.parsingKey = "mumin"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "mumin = {muminval} "
        self.interpreterQuickHelp = "model parameter Mu min"
        self.interpreterLongHelp = "Arora model parameter Mu min"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Electron Mobility,mumin,double,{muminval}")
        ...
        self.xmlRequiredArgument.append("{muminval}")
