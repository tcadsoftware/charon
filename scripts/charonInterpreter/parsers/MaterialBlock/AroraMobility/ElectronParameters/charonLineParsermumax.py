...
class charonLineParsermumax:
    "mumax parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "mumax"
        self.parsingKey = "mumax"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "mumax = {mumaxval} "
        self.interpreterQuickHelp = "model parameter Mu max"
        self.interpreterLongHelp = "Arora model parameter Mu max"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Closure Models->{MaterialBlockName}->Electron Mobility,mumax,double,{mumaxval}")
        ...
        self.xmlRequiredArgument.append("{mumaxval}")
