...
class charonLineParserBlockPhysicsMap:
    "BlockPhysicsMap parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "BlockPhysicsMap"
        self.parsingKey = "blockphysicsmap"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "blockPhysicsMap geometryBlock={geometryBlockName} physicsBlockName={physicsbBlockName} "
        self.interpreterQuickHelp = " Create a map between geometry blocks and the physics they contain"
        self.interpreterLongHelp = "  Create a map between geometry blocks and the physics they contain <> geometryBlockName is the geometry block as named in the mesh <> physicsBlockName is the physics information defined in the Charon interpreter file"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Block ID to Physics ID Mapping,{geometryBlockName},string,{physicsBlockName}")
        ...
        self.xmlRequiredArgument.append("geometryBlock={geometryBlockName}")
        self.xmlRequiredArgument.append("physicsBlockName={physicsbBlockName}")
