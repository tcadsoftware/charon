...
class charonLineParserResistorContact:
    "ResistorContact parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "ResistorContact"
        self.parsingKey = "bc is resistor for"
        ...
        self.interpreterHelpLine = "BC is resistor for {sidesetID} on {geometryBlock} with resistor {resistance} fixed at {voltage1} and initial voltage {voltage2} [with area {cntarea} [ and length {cntlength} [ with base doping type {baseType} ]]] "
        self.interpreterQuickHelp = "Specify a resistor boundary condition"
        self.interpreterLongHelp = "Specify a resistor boundary condition  <> sidesetID is the contact name/type <> geometryBlock is the geometry name the contact is attached to <> resistor is the value in ohms <> applied voltage is the value in volts <> initial voltage is the value in volts"
        ...
        self.xmlRequiredLines.append("Charon->Boundary Conditions->{sidesetID}ANONYMOUS,Strategy,string,Resistor Contact")
        self.xmlRequiredLines.append("Charon->Boundary Conditions->{sidesetID}ANONYMOUS->Data,Resistor Value,double,{resistance}")
        self.xmlRequiredLines.append("Charon->Boundary Conditions->{sidesetID}ANONYMOUS->Data,Applied Voltage,double,{voltage1}")
        self.xmlRequiredLines.append("Charon->Boundary Conditions->{sidesetID}ANONYMOUS->Data,Initial Voltage,double,{voltage2}")
        ...
        self.xmlRequiredArgument.append("{resistance}")
        self.xmlRequiredArgument.append("{voltage1}")
        self.xmlRequiredArgument.append("{voltage2}")
        ...
        self.xmlRequiredArgumentIndexes.append("12")
        self.xmlRequiredArgumentIndexes.append("16")
