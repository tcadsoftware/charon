...
class charonLineParserMass:
    "Mass parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "Mass"
        self.parsingKey = "mass"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "mass = {mass} "
        self.interpreterQuickHelp = "Set the barrier tunneling model mass value."
        self.interpreterLongHelp = "Set the barrier tunneling model mass value."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Boundary Conditions->SchottkyNeumann{sidesetID}ANONYMOUS->Data->Tunneling,mass,double,{mass}")
        ...
        self.xmlRequiredArgument.append("{mass}")
