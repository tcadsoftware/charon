...
class charonLineParserElectronRichardsonConst:
    "ElectronRichardsonConst parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "ElectronRichardsonConst"
        self.parsingKey = "electron richardson constant"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "Electron Richardson Constant = {An} "
        self.interpreterQuickHelp = "Sets electron Richardson constant for Schottky contact"
        self.interpreterLongHelp = "Sets electron Richardson constant for Schottky contact"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Boundary Conditions->SchottkyNeumann{sidesetID}ANONYMOUS->Data,Electron Richardson Constant,double,{An}")
        ...
        self.xmlRequiredArgument.append("{An}")
        ...
        self.xmlDefaultLines.append("Charon->Boundary Conditions->SchottkyNeumann{sidesetID}ANONYMOUS,Type,string,Neumann")
        self.xmlDefaultLines.append("Charon->Boundary Conditions->SchottkyNeumann{sidesetID}ANONYMOUS,Sideset ID,string,{sidesetID}")
        self.xmlDefaultLines.append("Charon->Boundary Conditions->SchottkyNeumann{sidesetID}ANONYMOUS,Element Block ID,string,{geometryBlock}")
        self.xmlDefaultLines.append("Charon->Boundary Conditions->SchottkyNeumann{sidesetID}ANONYMOUS,Equation Set Name,string,ELECTRON_DENSITY HOLE_DENSITY")
        self.xmlDefaultLines.append("Charon->Boundary Conditions->SchottkyNeumann{sidesetID}ANONYMOUS,Strategy,string,Neumann Schottky Contact")
