...
class charonLineParserBeta:
    "Beta parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "Beta"
        self.parsingKey = "beta"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "beta = {beta} "
        self.interpreterQuickHelp = "Set the barrier lowering model beta value."
        self.interpreterLongHelp = "Set the barrier lowering model beta value."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Boundary Conditions->SchottkyNeumann{sidesetID}ANONYMOUS->Data->Barrier Lowering,beta,double,{beta}")
        ...
        self.xmlRequiredArgument.append("{beta}")
