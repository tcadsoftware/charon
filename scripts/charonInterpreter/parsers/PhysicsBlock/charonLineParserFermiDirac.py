...
class charonLineParserFermiDirac:
    "FermiDirac parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "FermiDirac"
        self.parsingKey = "use fermi dirac"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("with schroeder")
        self.parsingKeyOptional.append("with diffusion")
        self.interpreterHelpLine = "use Fermi Dirac [ with Schroeder [ with Diffusion ]] "
        self.interpreterQuickHelp = "Turn on Fermi Dirac statistics"
        self.interpreterLongHelp = "Turn on Fermi Dirac Statistics <> with Schroeder formula"

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Physics Blocks->{physicsBlockName}->ANONYMOUS->Options,Fermi Dirac,string,True")
        ...
        self.xmlOptionalLines[0].append(" Charon->Physics Blocks->{physicsBlockName}->ANONYMOUS->Options,FD Formula,string,Schroeder")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[1].append(" Charon->Physics Blocks->{physicsBlockName}->ANONYMOUS->Options,FD Formula,string,Diffusion")
