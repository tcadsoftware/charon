...
class charonLineParserHarmonicBalanceAnalysis:
    "HarmonicBalanceAnalysis parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "HarmonicBalanceAnalysis"
        self.parsingKey = "apply harmonic balance"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("for large signal analysis")
        self.parsingKeyOptional.append("for small signal analysis")
        self.interpreterHelpLine = "apply harmonic balance [ for large signal analysis [ for small signal analysis]] "
        self.interpreterQuickHelp = "Set the problem in the frequency domain using the harmonic balance method. A discretization type needs to be specified."
        self.interpreterLongHelp = "Set the problem in the frequency domain using the harmonic balance method. A discretization type needs to be specified. <> Either 'for large signal analysis' or 'for small signal analysis' can be specified; default is large-signal analysis."

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Physics Blocks->{physicsBlockName}->ANONYMOUS,HBType,string,Frequency Domain")
        self.xmlRequiredLines.append("Charon->Physics Blocks->{physicsBlockName}->ANONYMOUS,Prefix,string,")
        self.xmlRequiredLines.append("Charon->Physics Blocks->{physicsBlockName}->ANONYMOUS->Frequency Domain Options,Hybrid Exponent,double,0.5")
        self.xmlRequiredLines.append("Charon->Physics Blocks->{physicsBlockName}->ANONYMOUS->Frequency Domain Options,Number of Time Collocation Points,int,0")
        self.xmlRequiredLines.append("Charon->Physics Blocks->{physicsBlockName}->ANONYMOUS->Frequency Domain Options,Enable Small Signal Analysis,bool,false")
        self.xmlRequiredLines.append("use Modifier 0")
        ...
        self.xmlOptionalLines[0].append(" Charon->Physics Blocks->{physicsBlockName}->ANONYMOUS->Frequency Domain Options,Enable Small Signal Analysis,bool,False")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[1].append(" Charon->Physics Blocks->{physicsBlockName}->ANONYMOUS->Frequency Domain Options,Enable Small Signal Analysis,bool,True")
