...
class charonLineParserInitialConditionsHarmonicBalance:
    "InitialConditionsHarmonicBalance parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "InitialConditionsHarmonicBalance"
        self.parsingKey = "harmonic balance initial conditions for"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("exodus file")
        self.parsingKeyOptional.append("default")
        self.parsingKeyOptional.append("constant")
        self.parsingKeyOptional.append("uninitialized")
        self.interpreterHelpLine = "Harmonic Balance Initial Conditions for {DOF} in {geometryBlock} is [ Exodus File[ default [constant = {constValue} [uninitialized ]]]] "
        self.interpreterQuickHelp = "Specify the source of the initial condition for the degrees of freedom"
        self.interpreterLongHelp = "Specify the source of the initial condition for the degrees of freedom <> Degrees of freedoms are <> ELECTRIC_POTENTIAL <> ELECTRON_DENSITY <> HOLE_DENSITY <> initial conditions are: Exodus File <> Constant <> Uninitialized"
        ...
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[2].append(" Charon->Initial Conditions->{geometryBlock}->{DOF},Value,double,{constValue}")
        self.xmlOptionalLines[2].append(" use Modifier 0")
        self.xmlOptionalLines.append([])
        self.xmlOptionalLines[3].append(" Charon->Initial Conditions->{geometryBlock}->ALL_FD_DOFS,Value,string,Frequency Domain Defaults")
        self.xmlOptionalLines[3].append(" Charon->Initial Conditions->{geometryBlock}->ALL_FD_DOFS,Maximum harmonic,int,5")
