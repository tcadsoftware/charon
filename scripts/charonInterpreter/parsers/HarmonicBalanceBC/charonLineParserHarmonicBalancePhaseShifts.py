...
class charonLineParserHarmonicBalancePhaseShifts:
    "HarmonicBalancePhaseShifts parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "HarmonicBalancePhaseShifts"
        self.parsingKey = "phase shifts"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "phase shifts {phaseshiftValues} "
        self.interpreterQuickHelp = "Specify the frequency domain boundary condition's phase shifts."
        self.interpreterLongHelp = "Specify the frequency domain boundary condition's phase shifts. <> phaseshiftValues is a list of phase shift (fraction of 1) values  formatted in double quotes with a space between each phase shift. For example  the input line can read <> phase shifts \"0  0.5  0.75\""

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Boundary Conditions->{sidesetID}ANONYMOUS->Data->Voltage,Phase Shifts,Array(double),{PHASESHIFTS{phaseshiftValues}PHASESHIFTS}")
        ...
        self.xmlRequiredArgument.append("{phaseshiftValues}")
