...
class charonLineParserHarmonicBalanceAmplitudes:
    "HarmonicBalanceAmplitudes parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "HarmonicBalanceAmplitudes"
        self.parsingKey = "amplitudes"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "amplitudes {amplitudeValues} "
        self.interpreterQuickHelp = "Specify the frequency domain boundary condition's amplitudes."
        self.interpreterLongHelp = "Specify the frequency domain boundary condition's amplitudes. <> amplitudeValues is a list of amplitude (in units appropriate to the BC type) values  formatted in double quotes with a space between each amplitude. For example  the input line can read <> amplitudes \"1.1e1  1.2e-2  1.0e-4\""

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Boundary Conditions->{sidesetID}ANONYMOUS->Data->Voltage,Amplitudes,Array(double),{AMPLITUDES{amplitudeValues}AMPLITUDES}")
        ...
        self.xmlRequiredArgument.append("{amplitudeValues}")
        self.xmlRequiredArgumentIndexes = []
        self.xmlRequiredArgumentIndexes.append("1")
