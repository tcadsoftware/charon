...
class charonLineParserHarmonicBalanceFrequencies:
    "HarmonicBalanceFrequencies parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "HarmonicBalanceFrequencies"
        self.parsingKey = "frequencies"
        self.parsingKeyOptional = []
        self.interpreterHelpLine = "frequencies {frequencyValues} "
        self.interpreterQuickHelp = "Specify the frequency domain boundary condition's frequencies."
        self.interpreterLongHelp = "Specify the frequency domain boundary condition's frequencies. <> frequencyValues is a list of frequency (Hz) values  formatted in double quotes with a space between each frequency. For example  the input line can read <> frequencies \"1e3  1.1e3  2.1e5\""

        # Register the xml required lines
        self.xmlRequiredLines = []
        self.xmlRequiredLinePriority = []
        self.xmlRequiredLines.append("Charon->Boundary Conditions->{sidesetID}ANONYMOUS->Data->Voltage,Frequencies,Array(double),{FREQUENCIES{frequencyValues}FREQUENCIES}")
        ...
        self.xmlRequiredArgument.append("{frequencyValues}")
