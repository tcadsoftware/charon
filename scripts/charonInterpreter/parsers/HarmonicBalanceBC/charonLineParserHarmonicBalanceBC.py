...
class charonLineParserHarmonicBalanceBC:
    "HarmonicBalanceBC parser"

    def __init__(self):
        # Register the parsing keys
        self.parserName = "HarmonicBalanceBC"
        self.parsingKey = "type is"
        self.parsingKeyOptional = []
        self.parsingKeyOptional.append("ohmic")
        self.interpreterHelpLine = "type is [ohmic] "
        self.interpreterQuickHelp = "Specify the frequency domain boundary condition."
        self.interpreterLongHelp = "Specify the frequency domain boundary condition on a particular contact. <> sidesetID is the contact name/type <> geometryBlock is the geometry name the contact is attached to <> bcStrategy can be 'Ohmic'"
        ...
        self.xmlOptionalLines[0].append(" Charon->Boundary Conditions->{sidesetID}ANONYMOUS->Data,Time Domain Strategy,string,Ohmic Contact")
