...
#   -DHDF5INCDIR:STRING=<HDF5 include directory>
#   -DHDF5LIBDIR:STRING=<HDF5 include directory>
#
...
SET(COMPILER "MPI_INTEL_14.x")

SET(DISTRIB "redsky")

# The location of extra software, namely netcdf, boost and HDF5
...
# RHEL 6.5
SET(CMAKEHOSTDEFS "-D BLAS_LIBRARY_NAMES:STRING=${BLASLIBRARYNAMES}")
SET(CMAKEHOSTDEFS "${CMAKEHOSTDEFS} -D BLAS_LIBRARY_DIRS:STRING=${BLASLIBRARYDIRS}")
SET(CMAKEHOSTDEFS "${CMAKEHOSTDEFS} -D LAPACK_LIBRARY_NAMES:STRING=${LAPACKLIBRARYNAMES}")
SET(CMAKEHOSTDEFS "${CMAKEHOSTDEFS} -D LAPACK_LIBRARY_DIRS:STRING=${LAPACKLIBRARYDIRS}")
...
if(${TYPE} MATCHES OPT)
  ...
  if(${osname} MATCHES "Darwin")
    SET(CTEST_CONFIGURE_COMMAND "${CTEST_CONFIGURE_COMMAND} -D CMAKE_CXX_FLAGS:STRING=-O3\\ \\ -w")
    SET(CTEST_CONFIGURE_COMMAND "${CTEST_CONFIGURE_COMMAND} -D CMAKE_C_FLAGS:STRING=-O3\\ -w")
  else()
    SET(CTEST_CONFIGURE_COMMAND "${CTEST_CONFIGURE_COMMAND} -D CMAKE_CXX_FLAGS:STRING=-O3\\ ")
  ...
  # there are too many of them
  if(${osname} MATCHES "Darwin")
    SET(CTEST_CONFIGURE_COMMAND "${CTEST_CONFIGURE_COMMAND} -D CMAKE_CXX_FLAGS:STRING=-g\\ \\ -w")
    SET(CTEST_CONFIGURE_COMMAND "${CTEST_CONFIGURE_COMMAND} -D CMAKE_C_FLAGS:STRING=-g\\ -w")
  else()
    SET(CTEST_CONFIGURE_COMMAND "${CTEST_CONFIGURE_COMMAND} -D CMAKE_CXX_FLAGS:STRING=-g\\ \\ -Wall")
...
SET(CTEST_CONFIGURE_COMMAND "${CTEST_CONFIGURE_COMMAND} -D TPL_ENABLE_HDF5:BOOL=ON")
if ("${HDF5INCDIR} " STREQUAL " ")
else()
  SET(CTEST_CONFIGURE_COMMAND "${CTEST_CONFIGURE_COMMAND} -D HDF5_INCLUDE_DIRS:PATH=${HDF5INCDIR}")
endif()
if ("${HDF5LIBDIR} " STREQUAL " ")
else()
  SET(CTEST_CONFIGURE_COMMAND "${CTEST_CONFIGURE_COMMAND} -D HDF5_LIBRARY_DIRS:PATH=${HDF5LIBDIR}")
endif()
...
SET(CTEST_CUSTOM_WARNING_EXCEPTION
  ...
  "Type mismatch in argument.*; passed"
  "printf\\(\"WARNING:"
  "boost/.*\\.hpp.*: warning:"
  "boost/.*\\.hpp.*: note:"
  "typedef.*boost.*locally defined but not used"
  "Trilinos/packages"
  "lex.yy.c"
  "aprepro.y"
  "aprepro.ll"
...
# Ignore warning about archives on redsky
if(CTEST_SITE MATCHES "redsky")
  SET(CTEST_CUSTOM_WARNING_EXCEPTION
    ${CTEST_CUSTOM_WARNING_EXCEPTION}
    "ipo: warning.*lib.*\\.a is an archive, but has no symbols")
endif()
